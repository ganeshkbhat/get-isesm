...
describe('test-.mjs::require-urls: Test Suite for require-urls Files', function () {
    describe('test-.js::require-urls: [Test A] Test Suite for require-urls in main repo directory', function () {
        ...
        it('[Test A] Test for file.mjs is a _isCJSCodeBase', function (done) {
            let c = parser._isCJSCodeBase("./demos/file.mjs");
            expect(c).to.equal(false);
            done();
        });

        it('[Test A] Test for file.mjs is a _isESMCodeBase', function (done) {
            let c = parser._isESMCodeBase("./demos/file.mjs");
            expect(c).to.equal(true);
            done();
        });

        it('[Test A] Test for file.mjs is a _isESCode', function (done) {
            let c = parser._isESCode("./demos/file.mjs");
            expect(c).to.equal(true);
            done();
        });

        it('[Test A] Test for file.cjs is a _isCJSCodeBase', function (done) {
            let c = parser._isCJSCodeBase("./demos/file.cjs");
            expect(c).to.equal(false);
            done();
        });

        it('[Test A] Test for file.cjs is a _isESMCodeBase', function (done) {
            let c = parser._isESMCodeBase("./demos/file.cjs");
            expect(c).to.equal(true);
            done();
        });

        it('[Test A] Test for file.cjs is a _isESCode', function (done) {
            let c = parser._isESCode("./demos/file.cjs");
            expect(c).to.equal(true);
            done();
        });

        it('[Test A] Test for file.js is a _isCJSCodeBase', function (done) {
            let c = parser._isCJSCodeBase("./demos/file.js");
            expect(c).to.equal(true);
            done();
        });

        it('[Test A] Test for file.js is a _isESMCodeBase', function (done) {
            let c = parser._isESMCodeBase("./demos/file.js");
            expect(c).to.equal(false);
            done();
        });

        it('[Test A] Test for file.js is a _isESCode', function (done) {
